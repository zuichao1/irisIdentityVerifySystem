...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    private void init() {
        ...
        String titleName = Constants.testEngineer+Constants.testReprotTitle;
        
        htmlReporter.config().setEncoding(Constants.htmlEncoding);
    ...
    private void buildTestNodes(ExtentTest extenttest, IResultMap tests, Status status) {
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                for(Object param:parameters){//这一块的作用是：将测试报告中每个测试用例的名称替换为参数
                    name+=param.toString();
                }
                if(name.length()>0){
                    if(name.length()>50){
                ...
                //设置suite内每个测试方法的描述
                test.getModel().setDescription(result.getMethod().getDescription().toString());
